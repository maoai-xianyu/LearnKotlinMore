package net.println.kotlinnew.chapter11.utils

import net.println.kotlinnew.chapter11.util.Log
...
object Logger {
    ...
    @JvmStatic
    fun debug(msg: Any?) {
        debugLogger("${header(Thread.currentThread().stackTrace.dropWhile { it.fileName != "Logger.kt" || it.methodName != "debug" }[1])} $msg")
    }

    @JvmStatic
    fun error(msg: Any?) {
        errorLogger("${header(Thread.currentThread().stackTrace.dropWhile { it.fileName != "Logger.kt" || it.methodName != "error" }[1])} $msg")
    }
}